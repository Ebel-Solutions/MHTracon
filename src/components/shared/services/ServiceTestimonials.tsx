...
export default function ServiceTestimonials({
  ...
  const rating = testimonials[current].rating ?? 5;

  return (
    <section
      className={`service-testimonials py-20 lg:py-28 bg-secondary-dark relative overflow-hidden ${className}`}
    >
      <div
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        >
          ...
          <h2 className="font-heading font-bold text-white text-3xl lg:text-4xl">{title}</h2>
        </motion.div>
        ...
        <div className="max-w-3xl mx-auto">
          <AnimatePresence mode="wait">
            <motion.div
              ...
              className="bg-white/5 border border-white/10 rounded-2xl p-5 sm:p-8 lg:p-12 text-center"
            >
              <Quote className="h-8 w-8 sm:h-10 sm:w-10 text-primary/60 mx-auto mb-4 sm:mb-6" />
              ...
              <blockquote className="font-body text-white/80 text-base sm:text-lg leading-relaxed italic mb-6 sm:mb-8">
            ...
            <div className="flex gap-2">
              {testimonials.map((_, i) => (
                <button
                  ...
                  className={`rounded-full transition-all duration-300 ${i === current ? 'w-6 h-2 bg-primary' : 'w-2 h-2 bg-white/30 hover:bg-white/60'}`}
