import type { Project } from '@/types';

export const projects: Project[] = [
  {
    id: 1,
    name: 'Commercial Office Interior — Riyadh',
    category: 'Interior Design',
    categorySlug: 'interior-exterior',
    imageSrc: '/images/banner/banner-2.png',
    link: '#',
  },
  {
    id: 2,
    name: 'MEP Installation — Industrial Facility',
    category: 'MEP Solutions',
    categorySlug: 'mep',
    imageSrc: '/images/banner/banner-3.png',
    link: '#',
  },
  {
    id: 3,
    name: 'Smart Security System — Corporate HQ',
    category: 'Security',
    categorySlug: 'security',
    imageSrc: '/images/banner/banner-5.png',
    link: '#',
  },
  {
    id: 4,
    name: 'AI Platform — Retail Analytics',
    category: 'AI & Software',
    categorySlug: 'ai-software',
    imageSrc: '/images/banner/banner-4.png',
    link: '#',
  },
  {
    id: 5,
    name: 'Luxury Hotel Exterior Renovation',
    category: 'Interior Design',
    categorySlug: 'interior-exterior',
    imageSrc: '/images/banner/banner-1.png',
    link: '#',
  },
  {
    id: 6,
    name: 'Corporate Uniform Program — 500+ Staff',
    category: 'Uniforms',
    categorySlug: 'uniforms',
    imageSrc: '/images/banner/banner-6.png',
    link: '#',
  },
];
